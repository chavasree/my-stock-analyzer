  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      ...
      if (idx.current >= text.length) {
        setDisplayed(text); setDone(true); clearInterval(iv);
      } else {
...
function RatingBadge({ rating }) {
  ...
  return (
    <span style={{
      ...
      fontWeight: 700, fontSize: '0.9em', letterSpacing: '0.05em'
    }}>{key}</span>
  );
}
  ...
  return (
    <div style={{ display: 'flex', height: '100%' }}>
      ...
      <div style={{
        ...
        background: '#080f0a', borderRight: '1px solid #1a2e1f',
        ...
        {SECTIONS.map((s, i) => (
          <button key={s.id} onClick={() => setActiveSection(i)}
            style={{
              ...
              padding: '7px 12px', background: activeSection === i ? '#0d2016' : 'transparent',
              border: 'none', borderLeft: activeSection === i ? '2px solid #00ff88' : '2px solid transparent',
              color: activeSection === i ? '#00ff88' : '#4a7a55',
              cursor: 'pointer', fontSize: '0.72em', fontFamily: 'inherit', transition: 'all 0.2s'
            }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '18px 22px' }}>
        <div style={{
          fontFamily: "'Courier New', monospace", fontSize: '0.8em',
          lineHeight: 1.85, color: '#c8e8d0', whiteSpace: 'pre-wrap', wordBreak: 'break-word'
        }}>
...
- Key Catalysts: [list]
- Invalidation: [list]
- Index Context (Nifty/Sensex): [current trend & impact on this stock]
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 52px)' }}>
      {step === 'input' && (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', overflowY: 'auto' }}>
          <div style={{ maxWidth: 640, width: '100%' }} className="fade-in">
            {/* Terminal window */}
            ...
            <div style={{ background: '#050d07', border: '1px solid #1a3d24', borderTop: 'none', borderRadius: '0 0 4px 4px', padding: '28px 26px' }}>
              <div style={{ marginBottom: 24 }}>
                <div style={{ color: '#00ff88', fontSize: '0.72em', letterSpacing: '0.1em', marginBottom: 6 }}>$ ./analyze --market india --mode full</div>
                <div style={{ color: '#00ff88', fontSize: '1.3em', fontWeight: 700, letterSpacing: '0.04em' }}>Stock Technical Analyzer</div>
                <div style={{ color: '#4a7a55', fontSize: '0.78em', marginTop: 4 }}>7-Step Deep Analysis · BSE · NSE · All Indian Indices</div>
              </div>

              <div style={{ marginBottom: 14 }}>
                <label style={{ display: 'block', color: '#4a7a55', fontSize: '0.68em', letterSpacing: '0.1em', marginBottom: 6 }}>STOCK NAME OR SYMBOL</label>
                <div style={{ display: 'flex', alignItems: 'center', background: '#0a1a0e', border: '1px solid #1a3d24', borderRadius: 3, padding: '0 12px' }}>
                  ...
                  <input ref={inputRef} value={stock} onChange={e => setStock(e.target.value)} onKeyDown={e => e.key === 'Enter' && runAnalysis()}
                    ...
                    style={{ flex: 1, background: 'transparent', border: 'none', color: '#c8e8d0', padding: '11px 0', fontSize: '0.88em', fontFamily: 'inherit' }} />
                </div>
              </div>

              <div style={{ marginBottom: 14 }}>
                <label style={{ display: 'block', color: '#4a7a55', fontSize: '0.68em', letterSpacing: '0.1em', marginBottom: 6 }}>INDEX (OPTIONAL)</label>
                <select value={selectedIndex} onChange={e => setSelectedIndex(e.target.value)}
                  style={{ width: '100%', background: '#0a1a0e', border: '1px solid #1a3d24', color: '#c8e8d0', padding: '9px 12px', fontSize: '0.82em', fontFamily: 'inherit', borderRadius: 3 }}>
              ...
              <div style={{ marginBottom: 22 }}>
                <label style={{ display: 'block', color: '#4a7a55', fontSize: '0.68em', letterSpacing: '0.1em', marginBottom: 6 }}>TIMEFRAME</label>
                <div style={{ display: 'flex', gap: 7 }}>
                  {TIMEFRAMES.map(tf => (
                    <button key={tf} onClick={() => setTimeframe(tf)}
                      style={{ flex: 1, padding: '7px', fontSize: '0.72em', background: timeframe === tf ? '#0d2016' : 'transparent', border: `1px solid ${timeframe === tf ? '#00ff88' : '#1a3d24'}`, color: timeframe === tf ? '#00ff88' : '#4a7a55', borderRadius: 3, cursor: 'pointer', fontFamily: 'inherit' }}>{tf}</button>
                  ))}
                </div>
              </div>
              ...
              {error && <div style={{ marginTop: 14, padding: '9px 13px', background: '#1a0008', border: '1px solid #ff3355', color: '#ff3355', borderRadius: 3, fontSize: '0.78em' }}>⚠️ {error}</div>}
            </div>
          </div>
        </div>
      )}
      ...
      {step === 'result' && analysis && (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }} className="fade-in">
          <div style={{ background: '#050d07', borderBottom: '1px solid #0d2016', padding: '8px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
            <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
              ...
              <span style={{ color: '#2a5a35', fontSize: '0.72em' }}>{timeframe}</span>
            </div>
            <div style={{ display: 'flex', gap: 7 }}>
              ...
              <button onClick={() => {
                ...
                a.download = `${stock.toUpperCase()}_analysis_${Date.now()}.txt`; a.click();
              }}
